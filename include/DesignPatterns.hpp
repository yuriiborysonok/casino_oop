...
public:
    double calculatePayout(double betAmount) const override {
        return betAmount * 2.0; // Виграш на кольорі подвоює ставку
    }
};
...
public:
    double calculatePayout(double betAmount) const override {
        return betAmount * 36.0; // Виграш на числі x36
    }
};
...
// ==========================================
// 3. CREATIONAL PATTERN: FACTORY METHOD
// ==========================================
#include <string>
#include <stdexcept>
...
public:
    ...
    static std::unique_ptr<IGame> createGame(const std::string& type) {
        if (type == "Roulette") return std::make_unique<RouletteGame>();
        if (type == "Slots") return std::make_unique<SlotMachineGame>();
