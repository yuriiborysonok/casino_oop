...
public:
  PostgresDatabase(std::shared_ptr<ILogger> logger, const std::string &connStr)
    ...
    try {
      ...
      if (conn.is_open()) {
        this->logger->info("Connected to REAL PostgreSQL DB: " +
        ...
        try {
          ...
          W.exec("ALTER TABLE users ADD COLUMN IF NOT EXISTS balance_sweep "
                 "NUMERIC(15, 2) DEFAULT 2.0;");
        ...
        W.commit();
        this->logger->info("PostgreSQL Tables created & verified successfully "
                           "(with Auth support).");
      }
    } catch (const std::exception &e) {
      this->logger->error("Failed to initialize PostgreSQL: " +
                          std::string(e.what()));
    }
  }

  void saveTransaction(int userId, double amount, const std::string &type,
    ...
    try {
      ...
      W.exec_params("INSERT INTO transactions (user_id, amount, currency, "
                    "type) VALUES ($1, $2, $3, $4)",
                    userId, amount, currency, type);

      // 2. Оновлюємо баланс (UPDATE)
      std::string balance_col =
          (currency == "sweep") ? "balance_sweep" : "balance_gold";

      if (type.find("WIN") != std::string::npos ||
          type.find("INITIAL_DEPOSIT") != std::string::npos) {
        W.exec_params("UPDATE users SET " + balance_col + " = " + balance_col +
                          " + $1 WHERE user_id = $2",
                      amount, userId);
      ...
      W.commit();
      logger->info("PostgreSQL [REAL]: Saved transaction -> User: " +
                   std::to_string(userId) + " | Type: " + type +
                   " | Currency: " + currency +
                   " | Amount: " + std::to_string(amount));
    } catch (const std::exception &e) {
      logger->error("Database Error (saveTransaction): " +
                    std::string(e.what()));
    }
  }
  ...
  int authenticateUser(const std::string &username,
    ...
    try {
      ...
      pqxx::result res = W.exec_params("SELECT user_id FROM users WHERE "
                                       "username = $1 AND password_hash = $2",
                                       username, password);
  ...
  int socialLoginUser(const std::string &email,
    ...
    try {
      ...
      logger->info("PostgreSQL: Registered new OAUTH user '" + email +
                   "' via " + provider);
